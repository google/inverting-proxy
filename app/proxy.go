...
// Command proxy defines an App Engine app implementing an inverse proxy.
//
	...
	for {
		...
		default:
			if r, err := s.ReadResponse(ctx, backendID, requestID); err == nil {
				...
				log.Printf("Error waiting for a response: %v", err)
			}
			// Wait a bit before checking again
			time.Sleep(100 * time.Millisecond)
		}
...
func pendingHandler(ctx context.Context, s types.Store, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("Failed to serialize the requests for %q: %q", backendID, err.Error())
...
func deleteHandler(ctx context.Context, s types.Store, w http.ResponseWriter, r *http.Request) {
	...
	if err := s.DeleteOldRequests(ctx); err != nil {
		log.Printf("Failed to delete old request data: %q", err.Error())
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func forwardResponse(ctx context.Context, requestID string, w http.ResponseWriter, response *http.Response) {
	...
	if _, err := io.Copy(w, response.Body); err != nil {
		log.Printf("Failed to copy a response: %v", err)
	}
}
...
func proxyHandler(ctx context.Context, s types.Store, requestID string, w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("%s|%s|%s| starting...", r.Method, r.URL.Path, userEmail)

	cacheKey := fmt.Sprintf("cache:%q:%q", userEmail, r.URL.String())
	if r.Method == http.MethodGet {
		if response, err := readCachedResponse(ctx, cacheKey, r); err == nil {
			log.Printf("%s|%s|%s| returning cached response", r.Method, r.URL.Path, userEmail)
	...
	if err := r.Write(&requestBuffer); err != nil {
		log.Printf("Failed to serialize request: %q", err.Error())
	...
	if err != nil {
		log.Printf("Failed to serialize the body for request %q", err.Error())
		reportError(w, requestID, err, http.StatusInternalServerError)
		return
	}

	if _, err := postRequest(ctx, s, backendID, requestID, userEmail, requestBytes); err != nil {
		log.Printf("Failed to cache a new request request %q", err.Error())
		reportError(w, requestID, err, http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		log.Printf("Failed to receive the response for request %q: %q", requestID, err.Error())
	...
	if err != nil {
		log.Printf("Failed to read the response for request %q: %q", requestID, err.Error())
		reportError(w, requestID, err, http.StatusInternalServerError)
		return
	}
	if r.Method == "GET" && response.StatusCode == 200 && response.Header.Get("Cache-Control") == "" {
		if err := cacheResponse(ctx, cacheKey, responseBytes); err != nil {
			log.Printf("Failed to cache response: %q", err.Error())
		}
	}
	log.Printf("%s|%s|%s| returning backend response: %+v", r.Method, r.URL.Path, userEmail, response)
	forwardResponse(ctx, requestID, w, response)
}
...
func addBackendHandler(ctx context.Context, s types.Store, w http.ResponseWriter, r *http.Request) {
	...
	if err := s.AddBackend(ctx, backend); err != nil {
		log.Printf("Failed to add a new backend: %q", err.Error())
...
func listBackendsHandler(ctx context.Context, s types.Store, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("Failed to serialize the backends: %q", err.Error())
		http.Error(w, "Failed to list backends", http.StatusInternalServerError)
		return
	}
...
func deleteBackendHandler(ctx context.Context, s types.Store, w http.ResponseWriter, r *http.Request) {
	...
	if err := s.DeleteBackend(ctx, backendID); err != nil {
		log.Printf("Failed to delete the backend %q: %q", backendID, err.Error())
...
func handleAgentRequest(ctx context.Context, s types.Store, w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Invalid agent path: %q", r.URL.Path)
...
func isAdminRequest(ctx context.Context) bool {
	...
	oauthUser, err := user.CurrentOAuth(ctx, "https://www.googleapis.com/auth/cloud-platform")
	if err != nil {
		log.Printf("Failed to read the OAuth authorization header for an API call: %q", err.Error())
		return false
	}
...
func handleAPIRequest(ctx context.Context, s types.Store, w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Received an admin API request: %q", r.URL.Path)
	if !isAdminRequest(ctx) {
		log.Printf("Received an admin API request from a non-admin user")
	...
	if r.URL.Path == "/api/backends" {
		...
		log.Printf("Unsupported API method for %q: %q", r.URL.Path, r.Method)
	...
	log.Printf("Invalid API path: %q", r.URL.Path)
	http.Error(w, "Invalid request path", http.StatusNotFound)
	return
}
